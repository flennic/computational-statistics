...
Putting this together using the `sgn()` function we get the following:
...
Now we swap $y$ and $x$ the get the quantile function.
...
$$ y = 1-\frac{1}{2} e^{-\frac{x-\mu}{b}}  $$

$$x = 1-\frac{1}{2} e^{-\frac{y-\mu}{b}}$$
$$(x-1)*(-2) =  e^{-\frac{y-\mu}{b}}$$
$$ln(2-2x)*b = -y+\mu$$
$$ln(2-2x)*b -\mu = -y$$
$$y = -ln(2-2x)*b + \mu$$
...
Combining those two we define our quantile function with $\mu = 0$, $\alpha = 1$ and thus $b = \frac{1}{1} = 1$:
...
Let's implement the quantile function:
...
**Answer:** As we have not yet created a function for the PDF $DE(\mu,\alpha)$ we will do this now. We will swap $\alpha$ with $b$ to ensure consitency.
