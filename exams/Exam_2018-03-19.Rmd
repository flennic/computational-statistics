...
**Answer:** We can see that the proposal envolops the target distribution but has a large rejection are. That's why we reject $\frac{\text{#rejected}}{\text{#accepted} + \text{#rejected}}$ and here given by ``r length(ars_results[[2]]) / (length(ars_results[[1]])+length(ars_results[[2]]))`` of the samples.

If we compare the real `Gamma(2, 2)` with the histrogram of our samples, they look almost identical-

*In the exam a plot of a histrogram of the real `Gamma(2, 2)` should be shown as well.*
