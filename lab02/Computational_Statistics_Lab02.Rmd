...
$$\frac{\partial\mathcal{L}(\mu, \sigma^2, x_1,...,x_{100})}{\partial\mu} = -\frac{1}{\sigma^2}\sum_{j=1}^n(x_j-\mu)$$
...
neg_llik_norm_prime = function(par) {
    mu_prime = -1/(par[2]) * sum(data-par[1])
    sigma_prime = 1/(2*par[2]^2) * (nrow(as.matrix(data)) -
    (1/(par[2]^4)) * sum((data-par[1])^2))
