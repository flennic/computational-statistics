...
Looking at the graph it indeed seems like we were able to reduce the noise. It's hard to say if the expected value of $\vec{\mu}$ is able to catch the true underlaying dependence between Y and X, but it looks like, as we have lees noise, which is some (distributed) error, that it does.
...
We see that the burn-in period is rather short (roughly `50` to `100` iterations) and that $\mu_n$ has converged after the burn-in. The trace jumps around the mean which is close to `1.5`.
