...
f = function(data, ind) {
  ...
  T_value = test_statistics(data1$Day_of_year, data1$Draft_No, Y_hat = model$fitted)
...
nonparam_bootstrap = boot(lottery, statistic = f, R = b, parallel = "multicore")
p_value_original = sum(nonparam_bootstrap$t > T_value_original) / b
...
ggplot(df) +
  geom_density(aes(x = nonparam_bootstrap.t), color = "#c70039", fill = "#C70039", alpha = 0.25) +
