...
p2.1 = ggplot(lottery)+
  geom_point(aes(x = Day_of_year, y = Draft_No))+ 
  geom_smooth(mapping = aes(x = Day_of_year, y = Draft_No),method = "loess", size = 1.5 )
p2.1
```

**Answer:**

There are two reasons, why now the lottery doesn`t look like random anymore:

1. the confidence interval of the curve is relatively small, especially compared to the spread of the data points.
2. the data points follow the pattern of the curve slightly, especially above the line. 
...
$$\text{, where } X_{b} = argmax_{X}Y(X),  X_{a} = argmin_{X}Y(X)  $$
